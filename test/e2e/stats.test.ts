...
describe("codex-wrapped e2e", () => {
  ...
  it("prints full stats JSON for 2026 when --year is provided", () => {
    const root = process.cwd();
    const cliPath = resolve(root, "dist", "index.js");
    const fixturesPath = resolve(root, "test", "fixtures", ".codex");
    ...
    const result = spawnSync(
      "node",
      [cliPath, "--stats-full", "--year", "2026", "--codex-home", fixturesPath],
      {
        encoding: "utf8",
        env: {
          ...process.env,
    ...
    expect(stats.totalProjects).toBe(1);
    expect(stats.totalInputTokens).toBe(861053);
    expect(stats.totalCachedInputTokens).toBe(716672);
    expect(stats.totalOutputTokens).toBe(8493);
    expect(stats.totalReasoningTokens).toBe(6976);
    expect(stats.totalTokens).toBe(869546);
    expect(stats.topModels[0]?.id).toBe("gpt-5.2-codex");
    expect(stats.topModels[0]?.count).toBe(869546);
    expect(stats.topProviders[0]?.id).toBe("openai");
    expect(stats.topProviders[0]?.count).toBe(869546);
    expect(stats.dailyActivity).toEqual([["2026-01-01", 1]]);
    expect(stats.mostActiveDay).toMatchObject({
      date: "2026-01-01",
      count: 1,
    });
    ...
    expect(stats.maxStreakDays).toEqual(["2026-01-01"]);
  });
